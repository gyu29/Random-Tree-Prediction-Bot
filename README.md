...
**One of these eight models does not produce a usable signal, and the app says so.** Each category is checked against the alternative of ignoring it entirely: bin every trade the model would open with no threshold at all by its predicted probability, and see whether higher-probability trades actually earn more (`scripts/expected_value_thresholds.py`). One of the eight fails that check today — `small_cap`, whose ranking is inverted — and is listed in `app.config.CATEGORIES_FAILING_VALIDATION`. They still load and can still be analyzed, because investigating a model means being able to run it, but they raise no alerts, take no place in the screener ranking, and carry a warning on every result. Of the seven that pass, only three clear two standard errors over the alternative of ignoring them; the config records the strength of each. Treat all of it as research output, not advice.
