...
**Three of these eight models do not produce a usable signal, and the app says so.** Each category is checked against the alternative of ignoring it entirely: bin every trade the model would open with no threshold at all by its predicted probability, and see whether higher-probability trades actually earn more (`scripts/expected_value_thresholds.py`). Three of the eight fail that check today and are listed in `app.config.CATEGORIES_FAILING_VALIDATION`. Two fail on significance rather than sign — their edge over doing nothing is real-looking but not established once the search for a threshold is paid for — and one, `small_cap`, ranks its best trades worst. They still load and can still be analyzed, because investigating a model means being able to run it, but they raise no alerts, take no place in the screener ranking, and carry a warning on every result. The config records, per category, the separation achieved and the bar it had to clear. Treat all of it as research output, not advice.
